...
window.addEventListener('load', initMap);
  ...
  const PHOTO_DEFAULT_NAME = 'No file chosen';
  const PHOTO_MAX_MB = 10;

  function updateFileName(text, isError = false) {
    if (!photoFileName) return;
    photoFileName.textContent = text;
    photoFileName.classList.toggle('is-error', Boolean(isError));
  }
  ...
  function assignInputFile(file) {
    ...
    try {
      ...
      /* ignore if browser does not permit programmatic assignment */
    }
  }
  ...
  function isTooLarge(file) {
    return Boolean(file && file.size > PHOTO_MAX_MB * 1024 * 1024);
  }

  async function handlePhotoFile(file, { syncInput = false } = {}) {
    if (!file) return false;
    if (!isImageFile(file)) {
      updateFileName('Please select an image file.', true);
      return false;
    }
    if (isTooLarge(file)) {
      updateFileName(`File too large (> ${PHOTO_MAX_MB}MB).`, true);
      return false;
    }
    ...
    setPhoto(dataUrl);
    if (syncInput) assignInputFile(file);
    return true;
  }

  if (photoUploader) {
    const onKeyActivate = (evt) => {
    ...
    photoUploader.addEventListener('click', openPhotoPicker);
    photoUploader.addEventListener('keydown', onKeyActivate);

    let dragDepth = 0;
    const setDragging = (active) => photoUploader.classList.toggle('dragging', active);

    ['dragenter', 'dragover'].forEach(evtName => {
      photoUploader.addEventListener(evtName, (evt) => {
        evt.preventDefault();
    ...
    photoUploader.addEventListener('drop', async (evt) => {
      evt.preventDefault();
      dragDepth = 0;
      setDragging(false);
      const dropped = evt.dataTransfer?.files?.[0];
      if (!dropped) return;
      const ok = await handlePhotoFile(dropped, { syncInput: true });
      if (!ok && photoInput) photoInput.value = '';
    });
  }

  photoChooseBtn?.addEventListener('click', (evt) => {
    evt.preventDefault();
    evt.stopPropagation();
    openPhotoPicker();
  });
  ...
  clearPhoto?.addEventListener('click', () => {
    setPhoto(null);
    updateFileName(PHOTO_DEFAULT_NAME);
    if (photoInput) photoInput.value = '';
  });

  updateFileName(PHOTO_DEFAULT_NAME);
  try {
    const cached = localStorage.getItem('ner.photo');
    if (cached) {
      setPhoto(cached);
      updateFileName('Saved photo');
    }
  } catch {
    /* ignore */
  }
  ...
  function calculate() {
    ...
    function bootCapex() {
      ...
      const landlordFreeTISeries = Array.from({ length: scheduleMonths + 1 }, () => 0);
      if (llAllowanceApplied > 0 && llAllowTreatment === 'cash') {
        landlordFreeTISeries[0] = llAllowanceApplied;
      }
      const freeTIAllowanceSeries = [...landlordFreeTISeries];
    ...
    function buildMonthlyColumns(model, perspective) {
      ...
      const cashflowExtras = {
        landlordFreeTI: landlordFreeTISeries,
        freeTIAllowance: freeTIAllowanceSeries
      };
